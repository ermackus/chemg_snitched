...
public class MaterialProgressDrawable extends Drawable implements Animatable
{
    ...
    MaterialProgressDrawable(final Context context, final View mParent) {
        this.mAnimators = (ArrayList<Animation>)new ArrayList();
    ...
    private void setupAnimators() {
        final Ring mRing = this.mRing;
        final Animation mAnimation = new Animation(this, mRing) {
            final MaterialProgressDrawable this$0;
            final Ring val$ring;
            ...
            public void applyTransformation(final float n, final Transformation transformation) {
                if (this.this$0.mFinishing) {
                    ...
                    if (n <= 0.5f) {
                        this.val$ring.setStartTrim(startingStartTrim + (0.8f - minProgressArc) * MaterialProgressDrawable.MATERIAL_INTERPOLATOR.getInterpolation(n / 0.5f));
                    }
                    if (n > 0.5f) {
                        this.val$ring.setEndTrim(startingEndTrim + (0.8f - minProgressArc) * MaterialProgressDrawable.MATERIAL_INTERPOLATOR.getInterpolation((n - 0.5f) / 0.5f));
                    }
                    this.val$ring.setRotation(startingRotation + 0.25f * n);
                    this.this$0.setRotation(n * 216.0f + this.this$0.mRotationCount / 5.0f * 1080.0f);
                }
            }
        };
        ...
        mAnimation.setAnimationListener((Animation$AnimationListener)new Animation$AnimationListener(this, mRing) {
            final MaterialProgressDrawable this$0;
            final Ring val$ring;
            ...
            public void onAnimationRepeat(final Animation animation) {
                ...
                final Ring val$ring = this.val$ring;
    ...
    void applyFinishTranslation(final float n, final Ring ring) {
        ...
        final float n2 = (float)(Math.floor((double)(ring.getStartingRotation() / 0.8f)) + 1.0);
    ...
    float getMinProgressArc(final Ring ring) {
        return (float)Math.toRadians(ring.getStrokeWidth() / (ring.getCenterRadius() * 6.283185307179586));
    }
    ...
    private static class Ring
    {
        ...
        private void drawTriangle(final Canvas canvas, final float n, final float n2, final Rect rect) {
            if (this.mShowArrow) {
                ...
                final float n6 = (float)this.mArrowWidth;
                final float mArrowScale2 = this.mArrowScale;
                mArrow2.lineTo(n6 * mArrowScale2 / 2.0f, this.mArrowHeight * mArrowScale2);
        ...
        public void draw(final Canvas canvas, final Rect rect) {
            ...
            final float n = (mStartTrim + mRotation) * 360.0f;
            final float n2 = (this.mEndTrim + mRotation) * 360.0f - n;
        ...
        public void setInsets(final int n, final int n2) {
            ...
            if (mRingCenterRadius > 0.0 && n3 >= 0.0f) {
                ceil = n3 / 2.0f - mRingCenterRadius;
            }
            else {
